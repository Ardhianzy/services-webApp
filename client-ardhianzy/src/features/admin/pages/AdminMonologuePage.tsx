// src/features/admin/pages/AdminMonologuePage.tsx

import { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import { ROUTES } from "@/app/routes";
import {
  adminFetchMonologues,
  adminDeleteMonologue,
...
const AdminMonologuePage: React.FC = () => {
  const navigate = useNavigate();
  const [rows, setRows] = useState<MonologueDTO[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] =
    useState<StatusFilter>("ALL");
  const [selected, setSelected] = useState<MonologueDTO | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setError(
            e?.message || "Gagal memuat daftar monologues."
          );
        }
      } finally {
        if (!cancelled) {
  ...
  const handleDelete = async (id: string) => {
    const ok = window.confirm(
      "Yakin ingin menghapus monologue ini? Tindakan ini tidak bisa dibatalkan."
    );
    if (!ok) return;

    try {
      await adminDeleteMonologue(id);
      setRows((prev) => prev.filter((m) => m.id !== id));
      setSelected((prev) => (prev && prev.id === id ? null : prev));
    } catch (e: any) {
      alert(e?.message || "Gagal menghapus monologue.");
    }
  };
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-neutral-400 mt-3 max-w-xl">
            Kelola monologues yang tampil di halaman Monologues.
            Setiap entri dapat memiliki dialog HTML dan file PDF
            untuk diunduh pembaca.
          </p>
        </div>
        <button
          type="button"
          onClick={() => navigate(ROUTES.ADMIN.MONOLOGUES_ADD)}
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1.1fr)]">
        <div className="bg-zinc-950/60 border border-zinc-800 rounded-3xl py-4 px-3">
          ...
          {loading ? (
            ...
            <p className="text-sm text-neutral-400">
              Belum ada monologue yang sesuai filter.
            </p>
          ) : (
                          ...
                          <div className="inline-flex items-center gap-2">
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                navigate(
                                  ROUTES.ADMIN.MONOLOGUES_EDIT(m.id)
                                );
                              }}
        ...
        <div className="bg-zinc-950/60 border border-zinc-800 rounded-3xl py-6 px-4 overflow-hidden flex flex-col">
          ...
          {!selected ? (
            <p className="text-sm text-neutral-500">
              Pilih salah satu monologue di sebelah kiri untuk melihat
              preview dialog dan PDF-nya.
            </p>
          ) : (
            <div className="bg-black rounded-2xl border border-zinc-800/80 py-5 px-4 overflow-y-auto">
              ...
              {(() => {
                const m = selected;
                const isPublished = Boolean(m.is_published);
                const htmlDialog = normalizeBackendHtml(
                  m.dialog ?? ""
                );
...
export default AdminMonologuePage;