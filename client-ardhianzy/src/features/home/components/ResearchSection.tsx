...
const ResearchSection: FC = () => {
  ...
  const rightPreview = right
    ? makePreview(right.description ?? "", "", 1)
    : { preview: "", truncated: false };
  ...
  return (
    <section id="research" className="relative w-full overflow-hidden bg-black py-[120px]">
      <div className="relative z-[2] mx-auto w-full max-w-[1560px] px-[80px] max-[1200px]:px-[40px] max-[767px]:px-[20px]">
        {/* =========================
        ...
        <div className="min-[768px]:hidden">
          <div className="res-slider relative z-[2]">
            {/* LEFT CARD */}
            <Link
              to={big ? `/research/${big.slug}` : ROUTES.RESEARCH}
              className={[
      ...
      <style>{`
        ...
        .res-slider { scrollbar-width: none; -ms-overflow-style: none; }
      `}</style>
    </section>
  );
};
